...
export function caffeineStatus(
  ...
  now: Date
): { totalMg: number; currentMg: number; hoursUntilNegligible: number } {
  let total = 0
  let current = 0
  ...
  const hoursUntilNegligible =
    currentMg <= NEGLIGIBLE_MG
      ? 0
      : Math.round(Math.log2(currentMg / NEGLIGIBLE_MG) * CAFFEINE_HALF_LIFE_HOURS * 10) / 10

  return { totalMg: Math.round(total), currentMg, hoursUntilNegligible }
}
